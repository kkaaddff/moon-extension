...
export class Initializer {
  ...
  private mappingSameFile(filePath: string) {
    let htmlPath: string = path.join(
      path.dirname(filePath),
      path.basename(filePath).replace(path.extname(filePath), ".html")
    );
    HtmlESMappingCache.addMapping(filePath, htmlPath);
  }
  ...
  private startWatching() {
    ...
    const pattern = path.join(
      this.rootPath,
      "/src",
      "**/*.{ts,js,html,css,less,scss,es}"
    );
    let watcher: FileSystemWatcher = workspace.createFileSystemWatcher(
      pattern,
      false,
      false,
      false
  ...
  private setESCache(content: string, filePath: string) {
    let info: ESFileInfo | null = ESFileAnalyzer.analyseESFile(
      content,
      filePath
    );
    if (!info) {
