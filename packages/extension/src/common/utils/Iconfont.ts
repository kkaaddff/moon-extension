            ...
            classUrlMap.forEach((url, className) => {
              this.fetchSvgData(className, url).then((list: any) => {
                ...
                list.forEach((data: any) => {
                  if (
                    !Iconfont.IconFontDataCache.find((item: any) => {
                      return (
      ...
      if (
        ...
        node.block.children.forEach((subNode: any) => {
          if (subNode.type === 'Declaration') {
            if (subNode.property === 'font-family' && subNode.value && subNode.value.children) {
            ...
            if (subNode.property === 'src' && subNode.value && subNode.value.children) {
              subNode.value.children.forEach((thNode: any) => {
                if (
                  thNode.type === 'Url' &&
                  thNode.value &&
